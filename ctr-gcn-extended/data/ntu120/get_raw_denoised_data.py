...
def denoising_by_spread(ske_name, bodies_data):
    ...
    for (bodyID, body_data) in new_bodies_data.items():
        if len(bodies_data) == 1:
            break
        valid_frames = get_valid_frames_by_spread((body_data['joints']).reshape(-1, 17, 4))
...
def get_one_actor_points(body_data, num_frames):
    ...
    """
    #colors = np.ones((num_frames[0].astype(int), 1, 17, 4), dtype=np.float32) * np.nan
    #start, end = body_data['interval'][0], body_data['interval'][-1]
    #colors[start:end + 1, 0] = body_data['joints']
    
    joints = np.zeros((num_frames[0].astype(int), 17 * 4), dtype=(np.float32))
    start, end = body_data['interval'][0], body_data['interval'][-1]
    joints[start:end + 1] = body_data['joints'].reshape(-1, 17 * 4)
